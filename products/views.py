...
def category_view(request, slug):
    """
    Display a category page for a collection identified by a slug.
    The slug is generated from the collection name with hyphens for spaces.
    """
    # convert slug to a name (replace hyphens with spaces) and try to match
    name = slug.replace('-', ' ')
    try:
        collection = CollectionCategory.objects.get(name__iexact=name)
        products = collection.product_set.all()
        coll_description = True
    except CollectionCategory.DoesNotExist:
        collection = None
        products = Product.objects.filter(upload_date__lte=timezone.now()).order_by('-upload_date')
        coll_description = False
